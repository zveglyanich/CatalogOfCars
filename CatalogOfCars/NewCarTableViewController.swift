...
class NewCarTableViewController: UITableViewController {
    ...
    @IBAction func saveButtonPressed(_ sender: UIBarButtonItem) {
        if let brand = brandTextField.text, let model = modelTextField.text, let year = Int(yearTextField.text!), let body = bodyTextField.text {
            if newCar == nil {
                ...
                guard let updatedCar = realm.objects(Cars.self).filter("id == \(newCar!.id)").first else { return }
                try! realm.write {
